...
export default function MenOutfits({ outfits }) {
    return (
        <Layout title="Women" description="Get ready to dive into the fabulous world of fashion with our featured collection which contains the best looking outfits for both men and women.">
            ...
            <section className="bg-dark-green text-creme px-4 md:px-10 pt-28 pb-12 lg:py-20 lg:pt-36">
                <h1 className="font-viaoda text-center text-5xl mb-14">Outfits for Women</h1>
                {outfits.length === 0 ? (
                    <h1 className="text-center text-2xl">No outfits were found :(</h1>
                ) : (
                    <div className="container mx-auto grid lg:grid-cols-2 xl:grid-cols-3 gap-10">
...
export async function getStaticProps() {
    const womenOutfits = outfits.filter(outfit => outfit.gender === "women")

    return {
        props: {
            outfits: womenOutfits,
        },
        revalidate: 900, 
    };
}
